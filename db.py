...
def public_text_is_safe(text, min_chars=6):
    s = str(text or "").strip()
    return bool(len(s) >= min_chars and not has_public_placeholder(s))
...
Write the article body only:"""
        ...
        body = strip_source_links(msg.content[0].text.strip())
        if not public_text_is_safe(body, min_chars=20):
            log.warning(f"Website Article Engine blocked unsafe body: {body[:90]}")
            return summary or title or ""
        return body
    except Exception as e:
...
def db_publish_article_for_website(article_id, title="", summary="", category="LOCAL",
    ...
    if not lang_ok:
        ...
        return
    if not public_text_is_safe(public_title) or has_public_placeholder(public_summary):
        log.warning(f"🌐 Website publish held — unsafe placeholder text: {public_title[:70]}")
        return
